<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 px-4 md:px-8">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 font-medium italic">Detalhes e informações de contato
                </p>
            </div>
    ...
    <div class="pt-6 pb-12 w-full max-w-7xl mx-auto">
        <div class="px-4 md:px-8 space-y-6">
            ...
            <div
                class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-sm p-6 overflow-hidden relative">
                <div class="absolute top-0 left-0 w-1 h-full {{ $customer->active ? 'bg-green-500' : 'bg-red-500' }}">
                </div>

                <div class="flex justify-between items-start mb-6 border-b border-gray-100 dark:border-gray-700 pb-4">
                    <h3
                        class="text-lg font-black text-gray-700 dark:text-gray-200 flex items-center gap-2 uppercase tracking-tighter">
                    ...
                    <div>
                        @if ($customer->active)
                            <span
                                class="px-3 py-1 text-[10px] font-black rounded-lg uppercase bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400">Ativo</span>
                        @else
                            <span
                                class="px-3 py-1 text-[10px] font-black rounded-lg uppercase bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400">Inativo</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Razão Social /
                            Nome</p>
                    ...
                    <div>
                        ...
                        <p class="text-base font-bold text-gray-800 dark:text-gray-200">
                            {{ $customer->name_fantasy ?? '—' }}</p>
                    </div>

                    <div>
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">CPF / CNPJ</p>
                        <p class="text-base font-bold text-gray-800 dark:text-gray-200">{{ $customer->cpf_cnpj ?? '—' }}
                        </p>
                    </div>

                    <div>
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Email</p>
                        <p class="text-base font-bold text-gray-800 dark:text-gray-200">{{ $customer->email ?? '—' }}
                        </p>
                    </div>

                    <div>
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Telefone</p>
                        <p class="text-base font-bold text-gray-800 dark:text-gray-200">{{ $customer->phone ?? '—' }}
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-base font-bold text-gray-800 dark:text-gray-200">
                            @if ($customer->website)
                                <a href="{{ str_starts_with($customer->website, 'http') ? $customer->website : 'https://' . $customer->website }}"
                                    target="_blank"
                                    class="text-orange-600 hover:underline">{{ $customer->website }}</a>
                            @else
                                —
                            @endif
                        </p>
            ...
            {{-- CARD 2: ENDEREÇO --}}
            <div
                class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl shadow-sm p-6 border-l-4 border-l-orange-600">
                <h3
                    class="text-lg font-black text-gray-700 dark:text-gray-200 mb-6 flex items-center gap-2 uppercase tracking-tighter border-b border-gray-100 dark:border-gray-700 pb-4">
                    <svg class="w-5 h-5 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                @if ($customer->address && ($customer->address->street || $customer->address->city))
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="md:col-span-2">
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Logradouro
                            </p>
                            <p class="text-base font-bold text-gray-800 dark:text-gray-200">
                                {{ $customer->address->street }}, {{ $customer->address->number }}</p>
                        </div>
                        ...
                        <div>
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Cidade /
                                Estado</p>
                            <p class="text-base font-bold text-gray-800 dark:text-gray-200">
                                {{ $customer->address->city ?? '—' }} / {{ $customer->address->state ?? '—' }}</p>
                        </div>
                        ...
                        @if ($customer->address->complement)
                            <div class="md:col-span-3 pt-4 border-t border-gray-100 dark:border-gray-700 mt-2">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">
                                    Complemento</p>
                                <p class="text-base font-medium text-gray-600 dark:text-gray-300 italic">
                                    {{ $customer->address->complement }}</p>
                            </div>
                        @endif
                    </div>
                @else
                    <div class="text-center py-6">
                        <p class="text-gray-500 dark:text-gray-400 font-medium">Nenhum endereço cadastrado para este
                            cliente.</p>
                    </div>
                @endif
            </div>
